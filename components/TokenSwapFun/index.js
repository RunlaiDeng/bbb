...
const TokenSwapFun = (props) => {
  ...
  let { index, symbol, imageUrl, token } = props;
  ...
  const buyTokenAmount = reads0?.[1]?.result;
  const sellXDCAmount = reads0?.[2]?.result;
  const klineLength = reads0?.[3]?.result;
  const tradeVolume = reads0?.[4]?.result;
  const tokenBalance = reads0?.[5]?.result || 0n;
