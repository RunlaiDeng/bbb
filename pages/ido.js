...
const Ido = () => {
  ...
  useEffect(() => {
    ...
    if (contractData && contractData[1]?.result && address) {
      setIsOwner(contractData[1].result.toLowerCase() === address.toLowerCase());
    }
        ...
        const campaignResults = await Promise.all(
          campaignIds.map(async (id) => {
            ...
            return {
              ...
              userPurchase,
              claimable,
              progress: campaign[4] > 0n ? 
                Number((campaign[5] * 10000n) / campaign[4]) / 100 : 0,
            };
          })
        );
  ...
  const getTimeStatus = (startTime, endTime, isActive, liquidityAdded) => {
    ...
    if (now < startTime) {
      return { status: 'upcoming', timeRemaining: formatTimeRemaining((startTime - now) * 1000) };
    } else if (now >= startTime && now <= endTime) {
      return { status: 'live', timeRemaining: formatTimeRemaining((endTime - now) * 1000) };
    } else {
      return { status: 'ended', timeRemaining: '0' };
    }
  };
  ...
  const handleCreateCampaign = () => {
    ...
    const startTimestamp = Math.floor(new Date(newCampaign.startTime).getTime() / 1000);
    const endTimestamp = Math.floor(new Date(newCampaign.endTime).getTime() / 1000);
  ...
  useEffect(() => {
    if (campaigns.length > 0) {
      const activeCampaigns = campaigns.filter(c => {
        const timeStatus = getTimeStatus(c.saleStartTime, c.saleEndTime, c.isActive, c.liquidityAdded);
        return timeStatus.status === 'live' && c.isActive;
      }).length;
      
      console.log("Campaign ethCollected values:", campaigns.map(c => ({ id: c.id, ethCollected: c.ethCollected, formatted: formatEther(c.ethCollected) })));
      const totalRaised = campaigns.reduce((acc, c) => acc + c.ethCollected, 0n);
    ...
    <div className="container mx-auto py-12 px-4">
      <div className="text-center mb-12">
        ...
        <p className="text-xl text-gray-600 max-w-3xl mx-auto">
          Participate in carefully vetted token launches and get early access to promising projects.
        </p>
      </div>
      ...
      {campaigns.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
          <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100 h-24 flex flex-col justify-between">
            ...
            <p className="text-3xl font-bold text-gray-800">{platformStats.totalCampaigns}</p>
          </div>
          <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100 h-24 flex flex-col justify-between">
            <p className="text-gray-500 text-sm">Active Campaigns</p>
            <p className="text-3xl font-bold text-gray-800">{platformStats.activeCampaigns}</p>
          </div>
          <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100 h-24 flex flex-col justify-between">
            <p className="text-gray-500 text-sm">Total Raised</p>
            <p className="text-3xl font-bold text-gray-800 truncate" title={`${formatEther(platformStats.totalRaised)} XDC`}>
              {parseFloat(formatEther(platformStats.totalRaised)).toFixed(2)} XDC
            </p>
          </div>
          <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100 h-24 flex flex-col justify-between">
            <p className="text-gray-500 text-sm">Average ROI</p>
            <p className="text-3xl font-bold text-green-600">{platformStats.averageRoi}%</p>
          </div>
        </div>
      )}
        ...
        <div className="mb-12">
          ...
          {showAdminPanel && (
            <div className="bg-white rounded-xl shadow-lg p-6 mb-8 border border-gray-200">
              <h2 className="text-2xl font-bold mb-4 text-green-600">Create New IDO Campaign</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Token Price (XDC)</label>
                  <input
                    type="number"
                    name="tokenPrice"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">XDC to Raise</label>
                  <input
                    type="number"
                    name="ethToRaise"
              ...
              <WriteButton
                ...
                disabled={!newCampaign.tokenName || !newCampaign.tokenSymbol || !newCampaign.tokenPrice || 
                          !newCampaign.totalSupply || !newCampaign.ethToRaise || !newCampaign.startTime || 
                          !newCampaign.endTime}
          ...
          {showAdminPanel && campaigns.length > 0 && (
            <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
              <h2 className="text-2xl font-bold mb-4 text-green-600">Manage Campaigns</h2>
                ...
                <table className="table-auto w-full">
                  ...
                  <tbody>
                    {campaigns.map((campaign) => {
                      const timeStatus = getTimeStatus(campaign.saleStartTime, campaign.saleEndTime, campaign.isActive, campaign.liquidityAdded);
                      ...
                      return (
                        <tr key={campaign.id} className="border-b border-gray-100">
                          ...
                          <td className="px-4 py-3">
                            <div>
                              <p className="font-medium">{campaign.tokenName}</p>
                              <p className="text-sm text-gray-500">{campaign.tokenSymbol}</p>
                            </div>
                          </td>
                          <td className="px-4 py-3">
                            <div className="flex items-center">
                              <span className={`inline-block w-2 h-2 rounded-full mr-2 ${
                                timeStatus.status === 'live' ? 'bg-green-500' : 
                                timeStatus.status === 'upcoming' ? 'bg-yellow-500' : 
                                timeStatus.status === 'inactive' ? 'bg-red-500' :
                                'bg-gray-500'
                              }`}></span>
                              <span className="capitalize">{timeStatus.status}</span>
                              <span className="ml-2 text-xs text-gray-500">{campaign.isActive ? '(Active)' : '(Inactive)'}</span>
                            </div>
                          </td>
                          <td className="px-4 py-3">
                            <div className="flex justify-between text-sm mb-1">
                              <span className="text-gray-600">Progress</span>
                              <span className="font-medium">{campaign.progress.toFixed(2)}%</span>
                            </div>
                            <div className="w-full bg-gray-200 rounded-full h-2">
                              <div 
                                className="bg-gradient-to-r from-green-500 to-emerald-600 h-2 rounded-full" 
                                style={{ width: `${Math.min(campaign.progress, 100)}%` }}
                                title={`${formatEther(campaign.totalTokensSold)} / ${formatEther(campaign.totalTokensForSale)} ${campaign.tokenSymbol}`}
                              ></div>
                            </div>
                            <div className="text-xs text-gray-500 mt-1 text-right">
                              {formatEther(campaign.totalTokensSold)} / {formatEther(campaign.totalTokensForSale)} {campaign.tokenSymbol}
                            </div>
                          </td>
                          <td className="px-4 py-3">
                            <div className="flex space-x-2">
                              <WriteButton
                                data={handleUpdateStatus(campaign.id, !campaign.isActive)}
                                buttonName={campaign.isActive ? "Deactivate" : "Activate"}
                                className="btn btn-xs btn-outline"
                                callback={handleSuccess}
                              />
                              
                              {timeStatus.status === 'ended' && !campaign.liquidityAdded && (
                                <WriteButton
      ...
      {!isConnected && !loading && (
        <div className="flex flex-col items-center justify-center py-16 mb-8 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl border border-green-100">
          ...
          <p className="text-gray-600 mb-6 text-center max-w-lg">
            Connect your wallet to view available IDO campaigns, purchase tokens, and track your investments.
          </p>
          <button
            ...
            onClick={() => {
              ...
              document.querySelector('header button[aria-label="Log in"]')?.click();
            }}
          >
            Connect Wallet
          </button>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-gray-50 rounded-lg">
          <div className="w-20 h-20 mx-auto mb-6 relative">
            <svg viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="5273" width="100%" height="100%">
            <path
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {campaigns.map((campaign) => {
            ...
            const campaignImage = `/ido/${campaign.tokenSymbol?.toLowerCase()}.png` || "/logo.png";
            ...
            return (
              ...
              >
                <div className="relative">
                  ...
                  <div className="absolute top-4 right-4">
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${
                      timeStatus.status === 'live' ? 'bg-green-100 text-green-800' : 
                      timeStatus.status === 'upcoming' ? 'bg-yellow-100 text-yellow-800' : 
                      timeStatus.status === 'ended' ? 'bg-gray-100 text-gray-800' :
                      'bg-red-100 text-red-800'
                    }`}>
                      <span className={`inline-block w-2 h-2 rounded-full mr-1.5 ${
                        timeStatus.status === 'live' ? 'bg-green-500 animate-pulse' : 
                        timeStatus.status === 'upcoming' ? 'bg-yellow-500' : 
                        timeStatus.status === 'ended' ? 'bg-gray-500' :
                        'bg-red-500'
                      }`}></span>
                      <span className="capitalize">{timeStatus.status}</span>
                    </span>
                ...
                <div className="p-5">
                  <h3 className="text-xl font-bold mb-1">{campaign.tokenName}</h3>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-gray-500">{campaign.tokenSymbol}</span>
                    {timeStatus.status === 'live' && (
                      <span className="text-sm text-green-600 font-medium">{timeStatus.timeRemaining} left</span>
                    )}
                    {timeStatus.status === 'upcoming' && (
                      <span className="text-sm text-yellow-600 font-medium">Starts in {timeStatus.timeRemaining}</span>
                    )}
                  </div>
                </div>
              </div>
      ...
      <div className="mt-24">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            ...
            <p className="text-gray-600">
              Connect your Ethereum wallet to browse available IDO campaigns and participate in token sales.
            </p>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            ...
            <p className="text-gray-600">
              Invest in promising projects by purchasing tokens during the IDO campaign with XDC.
            </p>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            ...
            <p className="text-gray-600">
              After the IDO concludes, claim your tokens and trade them on decentralized exchanges.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-16 bg-white p-8 rounded-xl shadow-sm border border-gray-100">
        <h2 className="text-3xl font-bold mb-6 text-center">How to Claim Your Tokens</h2>
        ...
        <div className="max-w-3xl mx-auto">
          <div className="space-y-6">
            <div className="flex items-start">
              ...
              <div>
                <h3 className="text-xl font-bold mb-2">Wait for the IDO to End</h3>
                <p className="text-gray-600">
                  Tokens become claimable only after the IDO campaign has ended and the project team has finalized the sale by adding liquidity.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start">
              ...
              <div>
                <h3 className="text-xl font-bold mb-2">Return to the IDO Page</h3>
                <p className="text-gray-600">
                  Once the IDO is finalized, return to this page and find your purchased tokens in the campaign card.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start">
              ...
              <div>
                <h3 className="text-xl font-bold mb-2">Click &quot;Claim Tokens&quot;</h3>
                <p className="text-gray-600">
                  In the campaign card where you purchased tokens, you&apos;ll see a &quot;Claim Tokens&quot; button when your tokens are ready to claim. Click this button and confirm the transaction in your wallet.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start">
              ...
              <div>
                <h3 className="text-xl font-bold mb-2">Add Token to Your Wallet</h3>
                <p className="text-gray-600">
                  After claiming, you may need to add the token to your wallet to see your balance. You can do this by adding the token contract address to your wallet&apos;s custom tokens.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-8 p-4 bg-yellow-50 rounded-lg border border-yellow-100">
            <div className="flex">
              ...
              <p className="text-sm text-gray-700">
                <span className="font-bold">Note:</span> The claim button will only appear when tokens are ready to be claimed. If you don&apos;t see it, either the IDO hasn&apos;t ended yet, or the project team hasn&apos;t finalized the sale.
              </p>
            </div>
          </div>
        </div>
